...
export default function Regis(props: IRegisProps) {
  const { setUser, setToken } = props;
  
  const [errTxt, setErrTxt] = useState<IErrorTxt>({
    email: [],
    password: [],
    confPass: [],
  });
  ...
  const submitHandler = async (e: FormEvent<HTMLFormElement> | MouseEvent<HTMLButtonElement, MouseEvent>): Promise<void> => {
    console.log(1);
    e.preventDefault();
    return;
    if (
    ...
    if (formData.name.length < 3) {
      toast.error("Your name is invalid");
      return;
    } 
    try {
